...
动态 OG 图片会在所有页面中使用, **除了**首页. 首页则使用在 [`ogImage`](/api/configuration/shadcn-docs#site) 字段中定义的静态 OG 图片.

## 使用内置模板

要使用模板, 请在 [`ogImageComponent`](/api/configuration/shadcn-docs#site) 中设置 OG Image 组件的名称.
...
::tabs{variant="card"}
  ::div{label="ShadcnDocs"}
    ![Shadcn Docs OG 图片预览](/og-shadcn-docs.png){width="1000"}
  ::
  ::div{label="ShadcnDocs (Dark)"}
    ![Shadcn Docs OG 图片预览](/og-shadcn-docs-dark.png){width="1000"}
  ::
  ::div{label="Nuxt"}
    ![Nuxt OG 图片预览](/og-nuxt.png){width="1000"}
  ::
  ::div{label="Nuxt SEO"}
    ![NuxtSEO OG 图片预览](/og-nuxt-seo.png){width="1000"}
  ::
::
