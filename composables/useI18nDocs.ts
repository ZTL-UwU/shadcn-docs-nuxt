...
export function useI18nDocs() {
    ...
    const localizedPrev = computed(() => {
        if (!i18nEnabled) return prev.value;
        if (locale.value === defaultLocale) {
    ...
    const localizedNext = computed(() => {
        if (!i18nEnabled) return next.value;
        if (locale.value === defaultLocale) {
     ...
     */
    const localePath = i18nEnabled ? useLocalePath() : (path) => path;
     ...
     */
    const localizeSearchResult = i18nEnabled ? (result) => result.filter(r => {
        if (locale.value === defaultLocale) {
